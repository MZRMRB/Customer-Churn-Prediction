...
def main():
    ...
    st.markdown("Enter the features below to predict whether the customer will churn.")

    user_input = {}
    for col in train_df.columns:
        if col != "Churn":
            if train_df[col].dtype == 'object':
                options = train_df[col].unique().tolist()
                user_input[col] = st.selectbox(f"{col}", options)
            else:
                user_input[col] = st.number_input(f"{col}", value=float(train_df[col].mean()))
